...
% function [sig] = ir_simple_matlab(h)
    ...
    disp('VS image reconstruction: simple matlab implementation');
    ...
    switch(h.format)
        ...
        case E.signal_format.RF
            ...
            wb=waitbar(0,'VS Beamforming, RF format');
            for f=1:h.F
                for nrx=1:h.M
                    waitbar(nrx/h.M);
                    RF=sqrt((h.geom(nrx,1)-r.x).^2+(h.geom(nrx,3)-r.z).^2);
                    for ntx=1:h.N
                        TF=sqrt((h.source(ntx,1)-r.x).^2+(h.source(ntx,3)-r.z).^2);
        ...
        case E.signal_format.IQ
            % beamforming
            sig=zeros(h.Nz,h.Nx,h.F);
            wb=waitbar(0,'VS Beamforming, IQ format');
