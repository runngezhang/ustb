...
void planewave_beamforming_rf_v4sf(const float* x_coords,const int Nx,const float* r_coords,const int Nr,float* ch_data,const int ch_data_sz[2],float* rf_bf_data,BF_Params params,const float* apodization)
{        
    v4si tix;
    ...
    for(int ix=0;ix<Nx;ix++)
    {
        ...
        for(int iz=0;iz<(Nr-offset);iz+=4)
        {
            ...
            {                    
                x_ch = _mm_set1_ps(ich*params.dx);                
                ...
                apod.m128f = _mm_set_ps(apodization[ii+3],apodization[ii+1],apodization[ii+2],apodization[ii]);
                ...
                tmp.m128f = _mm_add_ps(
                        _mm_add_ps(_mm_mul_ps(ctxa,z),_mm_mul_ps(stxa,xx_func(x,xd_sz))),
...
void planewave_beamforming_iq_v4sf(const float* x_coords,const int Nx,const float* r_coords,const int Nr,float* ch_dataRe,float* ch_dataIm,const int ch_data_sz[2],float* iq_bf_dataRe,float* iq_bf_dataIm,BF_Params params,const float* apodization)
{
    v4si tix;
    v4sf z,x,x0,rel_ap_sz,ti,ti_frac;
