...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray*prhs[] ) { 
	...
	if(nrhs==11) {
		F_number=*((double*)mxGetData(M_FNUMBER)); 
	}
	mexPrintf("F-number						%0.1f\n",F_number);
	double max_angle=1.0/2.0/F_number;
	mexPrintf("Max search angle:				%0.2f\n", max_angle*180/PI);
