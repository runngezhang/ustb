...
#define	M_SUM		prhs[6] // sum mode 0 -> NONE, 1->RX, 2->TX, 3->BOTH

// optional input
#define	M_VERBOSE	prhs[7] // verbose flag
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    ///////////////////////////////////////
    // number of inputs
    if (nrhs<7) mexErrMsgIdAndTxt("Toolbox:SRP_SRC:nrhs", "Too few input arguments");
    if (nrhs>8) mexErrMsgIdAndTxt("Toolbox:SRP_SRC:nrhs", "Too many input arguments");
    ...
    if (W!=W_check) mexErrMsgIdAndTxt("Toolbox:SRP_SRC:Dimensions", "Number of waves in channel data & apodization matrix do not match.");
    // single or double precision
    ...
    Concurrency::parallel_for(0, P, [&](int pp) { // pixel loop -> WIN
        ...
        tbb::strict_ppl::parallel_for(0, P, [&](int pp) { // pixel loop -> UNIX
            ...
            for (int rx = 0; rx<N; rx++) for (int w = 0; w<W; w++) { // channel & wave loop
                ...
                // references
                float& c_delay = *(p_delay + P*rx + PN*w + pp);      // delay
                float& c_apo = *(p_apo + P*rx + PN*w +pp);          // apodization
