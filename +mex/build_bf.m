function a=build_bf(use_v4sf,use_trig_tables,use_sse4,par_for)
% Function for building the planewave C++ functions for MATLAB. Several
% options are available for increase in speed.
%
% build_bf(use_v4sf,use_trig_tables,use_sse4,par_for)
% Input:
% use_v4sf          - If SSE3 extensions are available on your platform, set
%                     this flag to 1. Gives increase in speed but same accuracy 
%                     as standard c++ implementation.
...
%                     according to the applied delay and demodulation frequency. Can give
...
try
    cfiles = ['ta_source' filesep 'planewave_beamforming2.cpp '];
    d_flags = '';
    ld_flags = '';
    ...
    if use_v4sf
        ...
        eval(['mex -c ' d_flags ' ta_source' filesep 'planewave_bf_v4sf.cpp']);
        eval(['mex -c ' d_flags ' ta_source' filesep 'planewave_beamforming2.cpp']);
        
        cfiles = sprintf('planewave_beamforming2.%s planewave_bf_v4sf.%s',obj_ext,obj_ext);
    else                
        eval(['mex -c ' d_flags ' ta_source' filesep 'planewave_bf.cpp']);
        eval(['mex -c ' d_flags ' ta_source' filesep 'planewave_beamforming2.cpp']);
