classdef delay_multiply_and_sum_2 < process
    ...
    methods (Access = public)
        function h=delay_multiply_and_sum_2()
        ...
        function y_dmas_signed_img = delay_multiply_and_sum_implementation(h,data_cube,apod_matrix,progress)
            ...
            for z = 1:size(data_cube,1)
                ...
                for x = 1:size(data_cube,2)
                    ...
                    for i = idx(1:end-1)'
                        count = count + 1;
                        j_idx = idx(1+count:end)';
                        sisj_signed = sum(squeeze(sign(data_cube(z,x,i).*data_cube(z,x,j_idx)).*sqrt(abs(data_cube(z,x,i).*data_cube(z,x,j_idx)))));                      
