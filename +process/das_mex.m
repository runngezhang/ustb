        ...
        function out_data=go(h)
            ...
            if numel(mex.getCompilerConfigurations()) == 0 %Then no c/c++ compiler is set up.
                ...
                for n_wave=1:numel(h.channel_data.sequence)
                    ...
                    % das
                    out_data.data(:,1,n_wave,:)=mex.das_c(data(:,:,n_wave,:),sampling_frequency,initial_time,delay,apodization_matrix,modulation_frequency);
                    %out_data.data(:,1,n_wave,:)=reshape(mex.das_c(data(:,:,n_wave,:),sampling_frequency,initial_time,delay,apodization_matrix,modulation_frequency),[current_scan.N_pixels 1 1 h.channel_data.N_frames]);
                    %out_data.data(:,1,n_wave,:)=mex.das_c(data(:,:,n_wave,:),sampling_frequency,initial_time,delay,apodization_matrix,modulation_frequency);
                    
                    % assign phase according to 2 times the receive propagation distance
                    if(w0>eps)
                        out_data.data(:,1,n_wave,:)=bsxfun(@times,out_data.data(:,1,n_wave,:),exp(-1i*w0*2*rx_propagation_distance/h.channel_data.sound_speed));
                    end
                end
                tools.workbar(1);
            end
