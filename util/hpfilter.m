function [s,ds,desvabs] = hpfilter(y,w,weights,theta)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if ~exist('weights','var')
    ...
    d = repmat([w -4*w ((6*w)/2)], m, 1);
    d(1,2) = -2*w;      d(m-1,2) = -2*w;
    d(1,3) = (w)/2;   d(m,3) = (w)/2;
    d(2,3) = (5*w)/2; d(m-1,3) = (5*w)/2;
    
    L2 = spdiags(d, -2:0, m, m);    %I use a sparse version of B, because when m is large, B will have many zeros                 
    L2 = L2+L2';
