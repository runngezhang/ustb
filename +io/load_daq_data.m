...
p.addParamValue('Reroute',false);
% p.addParamValue('Size',[]);
p.addParamValue('WriteToFile',[]);
p.addParamValue('Precision','int16');
p.addParamValue('Channels',0:Nch-1);
p.addParamValue('FileNo',0);
p.addParamValue('FileFormat',6.0);
...
for fileno = res.FileNo(:)',
    ...
    for jj = 1:numel(ch),
        ...
        % close channel file
        % fclose(fid); % we need to keep this in addition to onCleanup
        % fprintf('%d of %d -- %s\n',jj,numel(ch),datestr(now,13));
    end 
end
