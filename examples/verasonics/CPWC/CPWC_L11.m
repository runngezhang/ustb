...
for n=1:na
    ...
    plot(probe_geometry(:,1),TX(n).Delay*lambda/Resource.Parameters.speedOfSound-t0_1(n),'r--'); grid on; hold on;
    plot(0,0,'bo');
    %pause();
end
...
delay_x0=sqrt(sum((probe_geometry-ones(128,1)*[0 0 20e-3]).^2,2))/c0;
...
probe.element_width = Trans.elementWidth;

% sequence
sequence = repmat(uff.wave(),[1 na]);
for n_angle = 1:na
    sequence(n_angle).source.distance = Inf;
...
channel_data.name = 'CPWC dataset recorded on Verasonics Vantage 256 and L11 probe';
