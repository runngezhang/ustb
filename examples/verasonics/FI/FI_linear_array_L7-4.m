...
filename = 'MatFiles/L7-4_128RyLns.mat';
save(filename)
VSX
...
%% SCAN
z_axis=linspace(0e-3,57e-3,512).';
sca=uff.linear_scan();
...
%% BEAMFORMER
%bmf=beamformer.beamformer(); %Potential new way of calling the beamforming
bmf=beamformer();
...
% beamforming
% b_data=bmf.go(postprocess.stack); %Potential new way of calling the
% beamforming
b_data=bmf.go(@matlab,@postprocess.stack);

%% show
b_data.plot(1,'DAS',80);

figure;
imagesc(db(abs(b_data.data./max(b_data.data(:)))));
colormap gray
caxis([-80 0])

figure;
plot(db(abs(b_data.data(:,19))));
