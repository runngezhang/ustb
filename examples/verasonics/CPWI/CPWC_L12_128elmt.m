...
mkdir(folderdata);            
filedata=['L12_CPW_' datestr(now,'HHMMSS') '.h5'];
...
aperture = 65;          % which part of the transducer to select the 128 elements from
                        % if aperture = 33 we use the central part.
                        % NB! The transducer at UiO has some dead elements,
                        % so you should use aperture = 65
...
SFormat.transducer = 'L12-3v';   % 192 element linear array with 0.96 lambda spacing
...
SFormat.rayDelta = Resource.Parameters.numTransmit*Trans.spacing;  % spacing in radians(sector) or dist. between rays (wvlnghts)
...
Resource.DisplayWindow(1).Title = 'L12-3v CPWC';
...
cpw_dataset = cpw();
cpw_dataset = ver.create_USTB_dataset(cpw_dataset);
...
cpw_image.name='CPW L12 example';                           % reconstruction name (optional)
if ismac
