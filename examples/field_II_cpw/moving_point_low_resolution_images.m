...
scan=linear_scan();
scan.x_axis=linspace(-5.1e-3,5.1e-3,64).';              % x vector [m]
scan.z_axis=linspace(15e-3,25e-3,64).';                 % z vector [m]
...
%% Low res
raw_data=s.low_res(recons);
reshaped_data=reshape(raw_data,[size(recons.scan.x_matrix,1) size(recons.scan.x_matrix,2) size(s.data,3) size(s.data,4)]);
