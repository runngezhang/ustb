%% Acoustical Radiation Force Imaging from UFF file recorded with the Verasonics ARFI_L7 example
%
% In this example we demonstrate the speckle tracking of displacement created
% by a shear wave induced with an acoustical "push" pulse. Also known as
% acoustic radiation force imaging. You will need an internet connection to 
% download the data. Otherwise, you can run the *ARFI_L7.m* Verasonics
% example and create your own .uff file.
%
...
% created by the share waves are to small to be seen. Notice that we barely
...
b_data.plot(1,['B-mode sqrt compression'],[30]);

%% Estimate displacement estimate with speckle tracking
% To actually see the share wave propagation we need to estimate the
% displacement. This is done using a USTB *process* called
% *pulsed_doppler_speckle_tracking*. Have a look at it's references to see
% the details.
...
% Now, we can finally show the estimated displacement. To see this we need
% to view it as a movie.
%
% In the movie we can clearly see the *acoustical radiation force* push
% that creates the share waves. The push is centered at x = 0 mm and 
% z = 14 mm. Notice how the sheare waves interacts with the harder
...
colormap(gca(f2),'jet');       % Changing the colormap
...
caxis([0.01*10^-6 0.3*10^-6]); % Updating the colorbar
colormap(gca(f3),'jet');       % Changing the colormap
