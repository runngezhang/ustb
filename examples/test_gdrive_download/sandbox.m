...
response = send(matlab.net.http.RequestMessage(), strcat(url, '/uc', '?', 'export=download', '&id=', id), opts);
...
if response.StatusCode == 200
    ...
    elseif strcmp(response.Body.ContentType.Type, 'text')
        ...
        % First, we prepare a second GET request, which will start the file download
        ...
        % We prepare a http FileConsumer object to take care of writing the file on
        % disk
        ...
        response = send(request, strcat(url, '/uc', '?', 'export=download', '&id=', id, '&confirm=', key), opts, consumer);
    else
