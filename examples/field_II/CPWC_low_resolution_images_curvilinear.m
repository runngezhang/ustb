...
% is beamformed in 3 different synthetic orientations. 
...
%Th = xdc_convex_focused_array (N_elements, width, height, kerf, 60e-3, 60e-3, noSubAz, noSubEl, [0 0 Inf]);
%Rh = xdc_convex_focused_array (N_elements, width, height, kerf, 60e-3, 60e-3, noSubAz, noSubEl, [0 0 Inf]);
...
figure;
plot(x0*1e3,z0*1e3); axis equal; grid on;
...
F_number=1.75;
rx_beam_angle=[-10 0 10]*pi/180;
for n=1:Na
    orien(n)=orientation();
    orien(n).transmit_beam=beam(F_number,E.apodization_type.none);  
    orien(n).receive_beam=beam(F_number,E.apodization_type.boxcar,rx_beam_angle(n));
end
...
%% beamform 
cpw_dataset.image_reconstruction(cpw_image,E.implementation.thor_andreas);
cpw_image.show();
