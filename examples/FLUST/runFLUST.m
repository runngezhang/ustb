...
s.firing_rate = 12000; % firing rate of output signal, not necessarily the same as Doppler PRF
s.nrReps = 10;         % nr of realizations 
s.nrSamps = 100;       % nr of slow time samples in each realization
s.overSampFact = 2;    % slow time overSampFact, high enough to avoid aliasing
s.dr = 5e-5;           % spatial discretization along flowlines: lambda/4 or smaller recommended if phase information is important

contrastMode = 0;      % this mode will simulate contrast scatterers propagating in flow field
...
for kk = 1:length(flowField),
    ...
    %% make realizations
    noAngs = size( PSFs.data, 3);
    for anglectr = 1:noAngs

        if isa( PSFs.scan, 'uff.sector_scan')
            szZ = length(PSFs.scan.depth_axis); % size( PSFs, 1);
            szX = length(PSFs.scan.azimuth_axis); % size( PSFs, 2);
        elseif isa( PSFs.scan, 'uff.linear_scan')
            szZ = length(PSFs.scan.z_axis); % size( PSFs, 1);
            szX = length(PSFs.scan.x_axis); % size( PSFs, 2);
        end

        PSFs_rsh = reshape( PSFs.data, [szZ, szX, noAngs, length( newtimetab)] );
        ...
        Nfft = 2*length(ts)+s.nrSamps*s.overSampFact*noAngs*s.nrReps+s.overSampFact*noAngs-1; %(length(ts)*(sparams.overSampFact+2) )-1;
        ...
        for coffset = 1:chunksize:szX,    
            ...
            myData_GPU = gpuArray( PSFs_rsh(:,cinds,anglectr,:) );
